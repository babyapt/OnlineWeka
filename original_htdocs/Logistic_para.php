...
switch ($test_option) {
	...
	case "Percentage split":
		$T_result = "-split-percentage ".$Percen." -t "." 2>errorcmd/".$SID.".txt";
		break;	
}
if(file_exists("savefile_pdf/Logistic_".$SID.".pdf"))
{
	unlink("savefile_pdf/Logistic_".$SID.".pdf");
}
if(file_exists("savefile_txt/Logistic_".$SID.".txt"))
{
	unlink("savefile_txt/Logistic_".$SID.".txt");
}
...
$cmd ="java -cp weka.jar weka.classifiers.functions.Logistic ".$result." ".$T_result." ".$File.$SID.".arff"." 2>errorcmd/".$SID.".txt";
	...
	if(sizeof($output) == 0)	// แม่ง Error
			...
			for ( $i = 0;$i<sizeof($output);$i++)
			{
				...
				file_put_contents("savefile_txt/Logistic_".$SID.".txt", $content,FILE_APPEND);
			...
			$pdf = new FPDF();	
	 		...
	 		$txt = file_get_contents("savefile_txt/Logistic_".$SID.".txt");
	 		$pdf->Write(5,$txt);
			$pdf->Output("savefile_pdf/Logistic_".$SID.".pdf","F");
...
<input name="Hidden" type="hidden" value="Logistic_">
...
<input name="Hidden2" type="hidden" value="Logistic_">	
<input name="btnSubmit" type="submit" value="Download PDF File" /></center></form>
</div>
