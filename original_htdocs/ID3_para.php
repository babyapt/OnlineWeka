...
if(file_exists("savefile_pdf/ID3_".$SID.".pdf"))
{
	unlink("savefile_pdf/ID3_".$SID.".pdf");
}
if(file_exists("savefile_txt/ID3_".$SID.".txt"))
{
	unlink("savefile_txt/ID3_".$SID.".txt");
}
...
$cmd ="java -cp weka.jar weka.classifiers.trees.Id3 ".$result." ".$T_result." ".$File.$SID.".arff"." 2>errorcmd/".$SID.".txt";
	...
	if(sizeof($output) == 0)	// แม่ง Error
			...
			for ( $i = 0;$i<sizeof($output);$i++)
			{
				...
				file_put_contents("savefile_txt/ID3_".$SID.".txt", $content,FILE_APPEND);
			...
			$pdf = new FPDF();	
	 		...
	 		$txt = file_get_contents("savefile_txt/ID3_".$SID.".txt");
	 		$pdf->Write(5,$txt);
			$pdf->Output("savefile_pdf/ID3_".$SID.".pdf","F");
...
<input name="Hidden" type="hidden" value="ID3_">
...
<input name="Hidden2" type="hidden" value="ID3_">	
