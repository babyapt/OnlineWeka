...
$distanceIsBranchLength = $_POST['distanceIsBranchLength'];
$linkType = $_POST['linkType'];
$numClusters = $_POST['numClusters'];
$printNewick = $_POST['printNewick'];
...
$result = " -N 2 -L"." ".$linkType." ".$printNewick." ".$debug." ".$distanceIsBranchLength." "."-A"." \"weka.core.".$distanceFunction." -R first-last\" ";
...
if(file_exists("savefile_pdf/HierarchicalClusterer_".$SID.".pdf"))
{
	unlink("savefile_pdf/HierarchicalClusterer_".$SID.".pdf");
}
if(file_exists("savefile_txt/HierarchicalClusterer_".$SID.".txt"))
{
	unlink("savefile_txt/HierarchicalClusterer_".$SID.".txt");
}
...
$cmd ="java -cp weka.jar weka.clusterers.HierarchicalClusterer".$result." ".$T_result." ".$File.$SID.".arff"." 2>errorcmd/".$SID.".txt";
	...
	if(sizeof($output) == 0)	// แม่ง Error
			...
			for ( $i = 0;$i<sizeof($output);$i++)
			{
				...
				file_put_contents("savefile_txt/HierarchicalClusterer_".$SID.".txt", $content,FILE_APPEND);
			...
			$pdf = new FPDF();	
	 		...
	 		$txt = file_get_contents("savefile_txt/HierarchicalClusterer_".$SID.".txt");
	 		$pdf->Write(5,$txt);
			$pdf->Output("savefile_pdf/HierarchicalClusterer_".$SID.".pdf","F");
...
<input name="Hidden" type="hidden" value="HierarchicalClusterer_">
...
<input name="Hidden2" type="hidden" value="HierarchicalClusterer_">	
